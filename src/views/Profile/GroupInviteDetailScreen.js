...
export class _GroupInviteDetailScreen extends React.Component {
    ...
    constructor(props) {
        super(props);
        // console.log("dazdzefzef : ", this.props.navigation.state);
        this.props.GetGroupInfo(this.props.navigation.state.params.invite.item.id);

    }


    render() {
        const group = this.props.groups[this.props.navigation.state.params.invite.item.id];
        // console.log("pd : ", this.props.navigation.state.params.group);
        if (group === undefined)
            return (<Loader/>);
        return (
            <View>
