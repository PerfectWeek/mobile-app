...
export class _EventDetailScreen extends React.Component {
  ...
  render() {
    ...
    if (present && user.status === "going") going = true;
    return (
        <Container
        style={{
        ...
        <ScrollView>
            ...
            <View style={rowStyle}>
              ...
              <Text style={{ fontSize: 18, marginLeft: 10 }}>
                {moment.utc(event.start_time).format("ddd., DD MMMM. HH:mm")} -{" "}
                {moment.utc(event.end_time).format("ddd., DD MMMM. HH:mm")}
              </Text>
            </View>
            <View style={rowStyle}>
              <Icon
            ...
            <View style={rowStyle}>
              ...
              >
                ...
                >
                  {i18n.t("other.addusers.seeattendees")}
                </Text>
              </TouchableOpacity>
            </View>
            {event.type !== "other" && (
              <View style={rowStyle}>
                <Icon
                  style={{ fontSize: 18 }}
                  active
                  name={type_to_icon[event.type]}
                  type={"MaterialCommunityIcons"}
                />
                <Text style={{ fontSize: 18, marginLeft: 10 }}>
                  {event.type}
                  {/* {i18n.t('dashboard.eventinfo.title')} */}
                </Text>
              </View>
            )}
          ...
          <Modal
            ...
            title={i18n.t("other.addusers.attendees")}
          >
            <UserList
              users={event.attendees.filter(a => a.status === "going")}
            />
          </Modal>
          ...
          >
            ...
            >
              <Button
                disabled={this.props.loading_joining === true}
                style={{
                  padding: this.props.loading_joining === true ? 10 : 0
                }}
                success={going}
                light={!going}
                onPress={() => {
                  if (!present) this.props.JoinEvent(event, "going");
                  else
                    this.props.ChangeEventStatus(event, going ? "no" : "going");
                }}
              >
                {this.props.loading_joining === true ? (
...
const mapDispatchToProps = (dispatch, ownProps) => {
  return {
    ...
    ChangeEventStatus: (event, status) =>
      dispatch(ChangeEventStatus(event, status))
  };
};
...
export const EventDetailScreen = connect(
  mapStateToProps,
  mapDispatchToProps
)(_EventDetailScreen);
