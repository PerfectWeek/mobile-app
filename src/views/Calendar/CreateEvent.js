...
export class _CreateEvent extends React.Component {
  static navigationOptions = {
    title:
      Localization.locale !== "fr-FR" ? "Create event" : "Créer l'événement"
  };
  ...
  async componentDidMount() {
    if (
      ...
      this.props.navigation.state.params.coordinate !== undefined
    ) {
      ...
      try {
        var res = await axios.get(
          `https://eu1.locationiq.com/v1/reverse.php?key=${geocofing_token}&lat=${lat}&lon=${lon}&format=json`
        );
  ...
  render() {
    ...
    if (
      this.props.calendar &&
      this.props.calendar.status === CalendarActionType.CreateNewEvent
    )
      return (
        <Container
          style={{
          ...
          <ScrollView>
            ...
            >
              ...
              <Item>
                ...
                >
                  <Picker.Item
                    label={i18n.t("dashboard.createvent.selectevent")}
                    value={-1}
                    key={-1}
                  />
                  {this.props.calendar.eventsType.map((type, index) => {
                    return (
                      <Picker.Item label={type} value={index} key={index} />
                    );
                  })}
                </Picker>
              </Item>
              ...
              >
                <Text>{i18n.t("dashboard.createvent.creat")}</Text>
              </Button>
            </Form>
          </ScrollView>
        </View>
