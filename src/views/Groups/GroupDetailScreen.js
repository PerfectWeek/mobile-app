...
export class _GroupDetailScreen extends React.Component {
  ...
  headerRightRender() {
    const group = this.props.groups.groups[
        this.props.navigation.state.params.group.id
      ];
      if (group.role !== "admin") return null;
    return (
      <Button
        transparent
        onPress={() => {
  ...
  render() {
    ...
    return (
      ...
      >
        <GroupDetailScreenGroupName
          group={group}
          onRef={ref => (this.groupName = ref)}
        />
        <GroupDetailScreenImagePicker
          group={group}
          onRef={ref => (this.groupImage = ref)}
        />
        <View>
          <Title
            style={{
          ...
          <List>
            {group.role === "admin" && (
              ...
              >
                ...
                >
                  <Icon
                    style={{ marginLeft: 10, color: Primary, fontSize: 32 }}
                    type="MaterialIcons"
                    name="group-add"
                  />
                  <Text style={{ marginBottom: 0, marginLeft: 30 }}>
                    {i18n.t("other.addusers.addmembers")}
                  </Text>
                </Body>
              </ListItem>
            )}
              ...
              return (
                <ListItem key={index} avatar>
                  <Left>
                    <Thumbnail
                      source={{
                        uri: `${axios.defaults.baseURL}/users/${
                          member.id
                        }/images/profile?rand=${uuidv4()}`
                      }}
                    />
                  </Left>
                  <Body>
        ...
        <AddUsers groupId={group.id} filterList={Object.values(group.members)} onRef={ref => (this.addUsers = ref)} />
      </ScrollView>
    );
  }
  ...
  ChangeRoleClicked(groupId, member) {
    this.props.UpdateMemberRole(
      groupId,
      member,
      member.role === "admin" ? "spectator" : "admin"
    );
  }
}
...
const mapDispatchToProps = (dispatch, ownProps) => {
  return {
    ...
    UpdateMemberRole: (groupId, member, newRole) =>
      dispatch(UpdateMemberRole(groupId, member, newRole)),
    RemoveGroupMember: (groupId, member, Selfpseudo) =>
      dispatch(RemoveGroupMember(groupId, member, Selfpseudo))
  };
};
...
export const GroupDetailScreen = connect(
  mapStateToProps,
  mapDispatchToProps
)(_GroupDetailScreen);
